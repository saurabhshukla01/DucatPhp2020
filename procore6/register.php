...
if(isset($register))
{
	...
	if($ext=="jpg" || $ext=="jpeg")
	{
		...
		if(move_uploaded_file($tmp,"users/".$fnn))
		{
			if(mysqli_query($link,"insert into users (name,email,password,mobile,address,city,pincode,image) values ('$un','$em','$pass','$mn','$add','$city','$pcode','$fnn')"))
			{
				header("location:login.php?user=$un");
			}
			else
			{
				unlink("users/".$fnn);
	...
	{
		$msg = "Only jpg or jpeg file support";
	}
}
