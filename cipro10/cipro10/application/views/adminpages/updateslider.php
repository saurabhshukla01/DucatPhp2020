...
<?= form_open_multipart("Admin/updateslidersave") ?>
				
		<?=form_input("hid",$sliderinfo->id)?>

		<div class="form-group">
		<?= form_label("Title 1","",["class"=>"text-primary"]) ?>
		<?= form_input("t1",$sliderinfo->title1,["class"=>"form-control"])?>	
		<?= form_error('t1') ?>
		</div>

		<div class="form-group">
		<?= form_label("Title 2","",["class"=>"text-primary"]) ?>
		<?= form_input("t2",$sliderinfo->title2,["class"=>"form-control"])?>	
		<?= form_error('t2') ?>
		</div>

		<?=form_input("himage",$sliderinfo->image)?>
		<div class="form-group">		
		<?= form_label("Image","",["class"=>"text-primary"]) ?>
		<img src="<?=base_url()?>uploads/slider/<?=$sliderinfo->image?>" height="30" width="80">
